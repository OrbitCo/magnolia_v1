(function($){
    $(function(){
        ...
        //get state from country chnage
        $( document ).on( 'change', "#my_country, #country", function(e) {
          
             Country_code = $(this).val();
             id = $(this).attr('id');
              var secondDropdown = $("#city"); // select 2nd dropdown
                secondDropdown.empty(); 
               $.ajax({
                    beforeSend: function() {
                    $(".tooltip_ajax").hover(function() {
                      $('.tooltiptext_ajax').css('visibility','visible');
                    });
                },
                type: 'POST',
                dataType: 'json',
                url: window.ajax + '/profile/get_city',
                data: {'country_code': Country_code},
               
                success: function(data) {
                    console.log(data.status);
                    console.log('city_length',data.city.length)
                    if( data.status == 200 ){
                        
                if(id=='my_country') {
                    option = '<option value=""> Choose your city </option>';
                } else {
                    option = '<option value="" disabled selected> Choose your city </option>';
                }
           ...
           $.each(data.city, function(key,value) {
                selected = " ";
            if(typeof window.cities!=undefined && window.cities!=0 && window.cities==value.id)
            {
                selected = "selected";
            }
            option+='<option value="'+ value.id +'" '+ selected +'>' + value.name + '</option>';
                        ...
                        $("#city").html(option).formSelect();
                        $(".tooltip_ajax").hover(function() {
                        $('.tooltiptext_ajax').css('visibility','hidden');
                      });
                    }
        ...
        $( document ).on( 'change', '[name="state"]', function(e){
             ...
             state_code = $(this).val();
               $.ajax({
                type: 'GET',
                url: window.ajax + '/profile/get_city',
                data: {'iso2': state_code},
                processData: true,
                success: function(data) {
                    if( data.status == 200 ){
                         var secondDropdown = $("#city"); // select 2nd dropdown
                secondDropdown.empty(); 
                        $("#city").append(data.cities).formSelect();
        ...
        $( document ).on( 'click', '.second', function(e){
            ...
            City_name = $( '#city' ).find(":selected").text();
        ...
        $( document ).on( 'change', '#is_my_location', function(e){
            if( $('#is_my_location').prop('checked') === false) {
               ...
               // $('#my_country').find('option[value="'+$('#my_country').attr('data-country')+'"]').prop('selected', true);
                ...
                //$('#city').find('option[value="'+$('#city').attr('data-city')+'"]').prop('selected', true);
                $('#city').formSelect();
