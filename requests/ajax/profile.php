...
//            }
    ...
    public function get_state()
    {
        ...
        if(isset($_GET['iso2']))
        {
           $states = $db->where('country_code',Secure($_GET['iso2']) )->get('states',null,array('name,iso2'));
           $option = '<option value="" disabled selected>'.__( 'Choose your state' ).'</option>';
           foreach($states as $key => $val )
           {
               $option.='<option value="'. $val['iso2'] .'" >'. $val['name'] .'</option>';
           }
            return array(
            'status' => 200,
            'states' => $option
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($error == '') {
                ...
                if (isset($_POST[ 'country' ])) {
                 ...
                 if(isset($_POST[ 'city' ]))
                {
                    $user[ 'cities' ] = Secure($_POST[ 'city' ]);

                    $city_name=$db->where('id',$user[ 'cities' ])->get('cities',null,array('name'));

                    $user[ 'cities_name' ] = Secure($city_name[0]['name']);
                    if($user['cities']==0)
                    {
                        $user['cities_name'] = 'Other';
                    }
                }
